...
def _run_dcr_vs_temp(svc: DataIngestionService, Vbr: float) -> dict:
...
def _run_pdp_vs_temp(svc: DataIngestionService, Vbr: float) -> dict:
    temps = np.array([285, 315])
    Vex = 3.0
...
def main() -> None:
    ...
    dcr_temp = _run_dcr_vs_temp(svc, Vbr)
    pdp_temp = _run_pdp_vs_temp(svc, Vbr)
