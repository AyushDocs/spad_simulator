...
class SPADSimulator:
    ...
    def __init__(
        ...
        poisson_solver = PoissonSolver(
            ...
            self.T,
            self.device.doping,
            self.device.material.eps,
            self.device.material.ni,
            max_iter=200,
        )
    ...
    def find_breakdown(
        ...
        if criterion == "current":
            ...
            def _compute_current(V: float, phi: np.ndarray, E: np.ndarray) -> float:
                ...
                J_total = self.dark_current.total_dark_current_density(
                    ...
                    self.device.material.mc,
                    self.device.material.mh,
                )
                # Trapezoidal rule: O(h²), handles non-uniform grid near heterojunctions
                I_primary = float(np.trapezoid(J_total, self.grid.x) * self.detector_area)
                return I_primary * M
        ...
        Vbr, results = self.poisson_service.find_breakdown(V_start, V_max, crit, V_step)
        if Vbr is None:
    ...
    def _trigger_for_pdp(self, E: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        alpha = self.ionization.alpha(E)
        beta = self.ionization.beta(E)
        Pe, Ph = self.trigger.solve(E, alpha, beta, self.grid.x, field_threshold=1e4)
        return Pe, Ph
    ...
    def compute_dark_current(self, Vbias: float, E: np.ndarray | None = None) -> dict:
        ...
        J_total = self.dark_current.total_dark_current_density(x, E, ni_arr, Eg_arr, mc_arr, mh_arr)
        # Trapezoidal rule: O(h²), handles non-uniform grid near heterojunctions
    ...
    def compute_photocurrent(
        ...
        dead_zone_layers, absorber = self.pdp_model.find_absorber(self.device.layers, "InGaAs")
    ...
    def compute_pdp_spectrum(
        ...
        dead_zone_layers, absorber = self.pdp_model.find_absorber(self.device.layers, material_name)
        dead_zone = sum(l.thickness for l in dead_zone_layers)
        L_abs = max(min(xr_loc - dead_zone, absorber.thickness), 0.0)
        ...
        for lam in wavelengths:
            ...
            pdp = self.pdp_model.pdp_integral(
                lam, xx, Ptr[mask], trans, self.grid.dx, material_name=material_name
            )
            pdp_vals.append(pdp)
    ...
    def run_mc_ensemble(
        ...
        self.mc_sim = MonteCarloSimulator(self.transport, self.ionization, self.grid)
        return self.mc_sim.run_ensemble(N_sim, x0, E, N_threshold=N_threshold, dt=dt)
