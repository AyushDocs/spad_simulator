...
def run_afterpulsing(sim: SPADSimulator, Vbr: float,
                     plot_cfg: PlotConfig | None = None) -> dict:
    if plot_cfg and not plot_cfg.is_enabled("afterpulsing"):
        return {}
...
def run_excess_noise(sim: SPADSimulator, Vbr: float,
                     plot_cfg: PlotConfig | None = None) -> dict:
    if plot_cfg and not plot_cfg.is_enabled("excess_noise"):
        return {}
    en = ExcessNoiseModel()
...
def run_breakdown_prob_vs_vex(sim: SPADSimulator, Vbr: float,
    ...
    """
    if plot_cfg and not plot_cfg.is_enabled("breakdown_prob_vs_vex"):
        return
...
def run_avalanche_current_pulse(sim: SPADSimulator, Vbr: float,
                                plot_cfg: PlotConfig | None = None) -> None:
    if plot_cfg and not plot_cfg.is_enabled("avalanche_current_pulse"):
        return
    Vex = 3.0
    try:
...
def run_quenching_waveform(sim: SPADSimulator, Vbr: float,
                           plot_cfg: PlotConfig | None = None) -> None:
    if plot_cfg and not plot_cfg.is_enabled("quenching_waveform"):
        return
    Vex = 3.0
    Vbias = Vbr + Vex
    try:
